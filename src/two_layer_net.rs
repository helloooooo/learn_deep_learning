...
impl Two_layer_network {
    ...
    pub fn loss(&self, param: &DMatrix<f64>,x: &DMatrix<f64>, t: &DMatrix<f64>,patern:&str) ->f64 {
        match patern{
            "w1" => {let y = predict(param,&*self.b1.borrow(),&*self.w2.borrow(),&*self.b2.borrow(),x);
                        lossfunc::cross_entropy(&y, t)},
            "b1" => {let y = predict(&*self.b1.borrow(),param,&*self.w2.borrow(),&*self.b2.borrow(),x);
                        lossfunc::cross_entropy(&y, t)},
            "w2" => {let y = predict(&*self.b1.borrow(),&*self.b1.borrow(),param,&*self.b2.borrow(),x);
                        lossfunc::cross_entropy(&y, t)},
            "b2" => {let y = predict(&*self.b1.borrow(),&*self.b1.borrow(),&*self.w2.borrow(),param,x);
                        lossfunc::cross_entropy(&y, t)},
            _ =>{ let y = self.predict(x);
                  lossfunc::cross_entropy(&y,t)
            }
        }
    }
...
pub fn loss_w(param: &DMatrix<f64>, x: &DMatrix<f64>, t: &DMatrix<f64>, two: &Two_layer_network,patern:&'static str) -> f64 {
    two.loss(param, x, t, &patern)
}
pub fn learn(late:&f64,matrix:&DMatrix<f64>) -> DMatrix<f64>{
    matrix.map(|i| i * late)
}
pub fn predict(w1:&DMatrix<f64>,b1:&DMatrix<f64>,w2:&DMatrix<f64>,b2:&DMatrix<f64>,x:&DMatrix<f64>) -> DMatrix<f64>{
    println!("{:?}",x.shape());
    println!("{:?}",w1.shape());
    println!("{:?}",b1.shape());
    let a1 = nural::dot(x,w1,b1);
